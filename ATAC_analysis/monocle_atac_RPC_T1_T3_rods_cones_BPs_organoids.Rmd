---
title: "Monocle pCM RPCs T1 T3 rods cones BPs - organoids"
output:
  html_document:
    df_print: paged
---
...
```{r, atac_subset}
##variables to change
pseudotime_order <- c("pCM", "early RPCs", "late RPCs", "T1", "T3", "cones", "rods", "BPS")

atacName <- "pCM_RPC_T1_T3_cones_rods_BPs"
atacTitle <- "pCM - early RPC - late RPC - T1 - T3 - cones - rods - BPs"
...
## RPCs T1 T3 trajectory
```{r}
start_Node = "Y_1"
```

The RPCs T1 T3 cell types were selected and Monocle 3 pseudotime analysis performe starting from `r start_Node_ATAC`.  
...
all <- data.frame(motif = unique(c(pCM,earlyRPCs,lateRPCs,T1,T3)))
